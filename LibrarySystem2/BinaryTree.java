...
public class BinaryTree<E> {
    ...
    public void breadthTraverse() {
        ...
        LinkedQueue<Node<E>> nodes = new LinkedQueue<>();
        nodes.enqueue(root);
    ...
    private void rangeSearchRecursive(Node<E> node, E min, E max, Comparator<E> c, MyArrayList<E> result) {
        if (node == null) {
            return;
        }

        // Pruning: only search left if min < node.value
        if (c.compare(min, node.value) < 0) {
            rangeSearchRecursive(node.left, min, max, c, result);
        }

        // Include node if it's within range
        if (c.compare(min, node.value) <= 0 && c.compare(node.value, max) <= 0) {
            result.add(node.value);
        }

        // Pruning: only search right if node.value < max
    ...
    // =========================================================
    // ✅ HEIGHT (حساب ارتفاع الشجرة)
    // =========================================================
    
    /**
     * حساب ارتفاع الشجرة
     * Height = أطول مسار من الـ root إلى أي leaf
     */
    public int height() {
    ...
    private int heightRecursive(Node<E> node) {
        ...
        // الارتفاع = 1 (العقدة الحالية) + أكبر ارتفاع من الفروع
        return 1 + Math.max(leftHeight, rightHeight);
    }
    
    // =========================================================
    // ✅ SIZE (عدد العناصر في الشجرة)
    // =========================================================
    
    /**
     * عدد العناصر في الشجرة
     */
    public int size() {
    ...
    // =========================================================
    // ✅ IS EMPTY (التحقق من فراغ الشجرة)
    // =========================================================
    
    /**
     * التحقق من فراغ الشجرة
     */
